...
- The exact local Emscripten staging manifest records GameLibs commit `0c9c121ff337b1c6df129ecedcfe569e7c50c332`, `gameLibsGitDirty: false`, `fileCount: 1361`, and the ignored source root `.tmp/pinned-gamelibs`. The selected checkout's `EULA.Development Kit.rtf` is retained outside Git and Docker; its SHA-256 is `d1f76da23b9ea17dc27773ad183cab74344c299c071f9b6b58c2e337e440b1a2`. The browser build's Emscripten compatibility edits are generated only in the ignored staging tree and are not written back to that checkout.
- The current checkpoint image includes the compiled SP/MP modules but intentionally exposes the gate instead of claiming clearance. This evidence binds the build input; it does not decide whether the modules may be distributed or whether the EULA must be included in the eventual image.

Decision status: OPEN. Before public publication, complete the staged-module and notice inventory and obtain the owner's distribution decision for the intended Docker Hub use.
